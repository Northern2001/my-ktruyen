...
export function MKTTrackDetail(props: Record<string, unknown>) {
  const {
    selectedProject,
    showOverlay,
    isDetailMinimized,
...
{selectedProject && (
      <div className={`detail-lyrics-layout ${isMobile ? "is-mobile" : ""} ${isDetailClosing ? "is-minimized" : ""} ${isDetailMinimizing ? "is-minimizing" : ""}`}>
        ...
        >
          ...
          {!isDetailMinimized && selectedTrack?.type === "pulled" && selectedTrack.lyrics && (!isMobile || (!isLyricsOpen && !isSongAnnotationOpen)) && (
            <button
